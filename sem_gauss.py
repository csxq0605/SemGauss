...
def get_dataset(config_dict, basedir, sequence, **kwargs):
    ...
    elif config_dict["dataset_name"].lower() in ["goat-core", "goat_core", "goatcore"]:
        return GoatCoreDataset(config_dict, basedir, sequence, **kwargs)
    else:
...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
    ...
    if tracking and visualize_tracking_loss and (iter_time_idx+1) % 500 == 0:
...
def dense_semantic_slam(config: dict):
    ...
    dataset = get_dataset(
        ...
        # Use the full relative path (e.g., "room_0/Sequence_1") instead of stripping
        # the parent folder; otherwise the dataset points to .../Sequence_1 and misses
        # the scene directory.
    ...
    if config['load_checkpoint']:
        ...
        # Loading the saved pose and representation of scene to rebuild keyframes if needed
        for time_idx in range(checkpoint_time_idx):
            ...
            if time_idx in keyframe_time_indices:
                ...
                seg_net.set_mode_get_feature()
                with torch.no_grad():
                    sem_feature = seg_net.cnn(rgb_input)  # [1,16,H,W]
                    sem_feature = sem_feature.squeeze(0).detach()  # [16,H,W]
    ...
    with torch.no_grad():
        eval(config, dataset, params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                 ...
                 eval_every=config['eval_every'], build_mesh=config.get('eval_mesh', True))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Replica scene index to override config scenes list.",
    )
    args = parser.parse_args()

    if args.scene_idx is not None:
        os.environ["SCENE_IDX"] = str(args.scene_idx)
