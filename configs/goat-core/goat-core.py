...
seed = 2027
scene_idx = int(os.environ.get("SCENE_IDX", 0))
if scene_idx < 0 or scene_idx >= len(scenes):
    raise ValueError(f"scene_idx out of range: {scene_idx} (0-{len(scenes)-1})")
...
config = dict(
    ...
    eval_mesh=True, # Skip TSDF mesh integration to reduce RAM usage
    scene_radius_depth_ratio=3, # Max First Frame Depth to Scene Radius Ratio (For Pruning/Densification)
    mean_sq_dist_method="projective", # ["projective", "knn"] (Type of Mean Squared Distance Calculation for Scale of Gaussians)
    report_iter_progress=False,
    load_checkpoint=True,
    checkpoint_time_idx=599,
    ...
    model=dict(
        ...
        # absolute path to local checkpoint; keep relative to repo root
        pretrained_model_path=os.path.abspath("./segmentation/replica/dinov2_replica.pth"),
    ...
    data=dict(
        # dataroot inside repo (see README: ./data0/replica by default)
        basedir=os.path.abspath("./data0/goat-core"),
        gradslam_data_cfg="./configs/data/goat-core.yaml",
        sequence=scene_name,
        # keep same resolution as model/weights expect
    ...
    mapping=dict(
        ...
        loss_weights=dict(
            im=0.5, #0.1 /1
            depth=1.0,  #0.5 /2
            se=0.0,  #use_F    #0.005 / 0.02
            se_fe=0.0,    #use_F #0.005 / 0.02
        ),
        lrs=dict(
            means3D=0.0001,
            rgb_colors=0.0025,
